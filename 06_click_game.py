...
class Points:
    def __init__(self):
        self.positionX = 0
        self.positionY = 0
        self.startMessage = "Points: "
        self.points = 0
        self.color = 7

    def draw(self):
        message = f"{self.startMessage}{self.points}"
        pyxel.text(self.positionX, self.positionY, message, self.color)
        ...
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
            for i in range(num_bubbles):
                ...
                if dx * dx + dy * dy < bubble.r * bubble.r:
                    ...
                    self.points.points += math.floor(bubble.r)

                    for j in range(NUM_EXPLODE_BUBBLES):
                        angle = 360 * j / NUM_EXPLODE_BUBBLES
                        angle = angle + pyxel.rndi(-180, 180)
